...
/** Deep link @rawlead_bot для оплаты: pay | pay_sbp | pay_crypto | pay_stars. */
function rawlead_bot_pay_url(string $method = 'pay'): string {
    $map = [
        'pay'    => 'pay',
        'menu'   => 'pay',
        'sbp'    => 'pay_sbp',
    ...
    $start = $map[$key] ?? 'pay';
    return esc_url('https://t.me/' . rawlead_tg_login_bot_username() . '?start=' . $start);
}
...
/** O116-W4: Bearer and/or guest token for support thread. */
function rawlead_api_support_headers(WP_REST_Request $request): array {
    $headers = ['Accept' => 'application/json'];
    $bearer = rawlead_api_bearer_from_request($request);
    if ($bearer !== '') {
        $headers['Authorization'] = 'Bearer ' . $bearer;
    }
    $guest = $request->get_header('x_rawlead_guest_token');
    if (!is_string($guest) || $guest === '') {
        $guest = $request->get_header('x-rawlead-guest-token');
    }
    if (is_string($guest) && $guest !== '') {
...
function rawlead_api_post(string $path, array $body, array $extra_headers = [], int $timeout = 20): array|WP_Error {
...
add_action('rest_api_init', static function (): void {
    ...
    register_rest_route('rawlead/v1', '/leads/(?P<id>\d+)/tg-spam', [
        ...
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            ...
            $data = rawlead_api_post('/v1/admin/leads/' . $id . '/tg-spam', [], $headers);
    ...
    register_rest_route('rawlead/v1', '/ops/proxies', [
        'methods'             => 'GET',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $data = rawlead_api_get('/v1/admin/proxies', [], false, $request);
            if (is_wp_error($data)) {
                return $data;
            }
            return new WP_REST_Response($data, 200);
    ...
    register_rest_route('rawlead/v1', '/ops/control', [
        ...
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            ...
            if ($target === 'delist') {
                $timeout = 120;
            } elseif ($target === 'proxy' && $action === 'probe-all') {
                $timeout = 60;
            } elseif (
                ($target === 'proxy' && $action === 'clear-bans')
                || (
                    $action === 'restart'
                    && (
                        $target === 'radar'
                        || $target === 'site'
                        || str_starts_with($target, 'bots-')
                        || $target === 'bots'
                    )
                )
            ) {
            ...
            $data = rawlead_api_post('/v1/admin/control', $body, $headers, $timeout);
            if (is_wp_error($data)) {
                return $data;
            }
            return new WP_REST_Response($data, 200);
    ...
    register_rest_route('rawlead/v1', '/site-stats', [
        'methods'             => 'GET',
        'permission_callback' => '__return_true',
        'callback'            => static function (): WP_REST_Response|WP_Error {
            $data = rawlead_api_get('/v1/public/site-stats');
            if (is_wp_error($data)) {
                return $data;
            }
            return new WP_REST_Response($data, 200);
    ...
    register_rest_route('rawlead/v1', '/me', [
        [
            'methods'             => 'GET',
            'permission_callback' => '__return_true',
            'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
                ...
                $url = rawlead_api_base_url() . '/v1/me';
                $response = wp_remote_get($url, [
                    'timeout' => 20,
                    'headers' => rawlead_api_user_headers($request, false),
                ]);
    ...
    register_rest_route('rawlead/v1', '/me/avatar', [
        [
            'methods'             => 'GET',
            'permission_callback' => '__return_true',
            'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
                ...
                $url = rawlead_api_base_url() . '/v1/me/avatar';
                $response = wp_remote_get($url, [
                    'timeout' => 20,
                    'headers' => rawlead_api_user_headers($request, false),
                ]);
                ...
                $code = (int) wp_remote_retrieve_response_code($response);
                if ($code < 200 || $code >= 300) {
                    return new WP_Error('rawlead_api_http', 'avatar unavailable', ['status' => $code]);
                }
                $body = wp_remote_retrieve_body($response);
                ...
                $rest = new WP_REST_Response($body, 200);
                $rest->header('Content-Type', $ctype);
                $rest->header('Cache-Control', 'private, max-age=3600');
                return $rest;
            },
        ],
    ]);
    ...
    register_rest_route('rawlead/v1', '/quiz/start', [
        'methods'             => 'GET',
        'permission_callback' => '__return_true',
        'callback'            => static function (): WP_REST_Response|WP_Error {
            $data = rawlead_api_get('/v1/quiz/start');
            if (is_wp_error($data)) {
                return $data;
            }
            return new WP_REST_Response($data, 200);
    ...
    register_rest_route('rawlead/v1', '/quiz/next', [
        'methods'             => 'POST',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $body = $request->get_json_params();
            ...
            $data = rawlead_api_post('/v1/quiz/next', $body);
            if (is_wp_error($data)) {
                return $data;
            }
            return new WP_REST_Response($data, 200);
    ...
    register_rest_route('rawlead/v1', '/me/tags/import', [
        'methods'             => 'POST',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            if (rawlead_api_bearer_from_request($request) === '') {
            ...
            $url = rawlead_api_base_url() . '/v1/me/tags/import';
            $response = wp_remote_post($url, [
                'timeout' => 20,
                'headers' => array_merge(
                    rawlead_api_user_headers($request, false),
    ...
    register_rest_route('rawlead/v1', '/me/tags/weight_delta', [
        'methods'             => 'POST',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            if (rawlead_api_bearer_from_request($request) === '') {
            ...
            if (!is_array($body) || !isset($body['event']) || !is_string($body['event'])) {
                return new WP_Error('rawlead_invalid', 'Expected {"event":"expand|draft|inbox_delete","tags":[]}', ['status' => 400]);
            }
            $tags = isset($body['tags']) && is_array($body['tags']) ? $body['tags'] : [];
            $url = rawlead_api_base_url() . '/v1/me/tags/weight_delta';
            $response = wp_remote_post($url, [
                'timeout' => 20,
                'headers' => array_merge(
                    rawlead_api_user_headers($request, false),
    ...
    register_rest_route('rawlead/v1', '/me/subscription/checkout', [
        [
            ...
            'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
                ...
                $kind = isset($body['kind']) ? (string) $body['kind'] : 'subscription';
                $url = rawlead_api_base_url() . '/v1/me/subscription/checkout';
                $response = wp_remote_post($url, [
                    'timeout' => 25,
                    'headers' => array_merge(
                        rawlead_api_user_headers($request, false),
    ...
    register_rest_route('rawlead/v1', '/me/subscription/confirm', [
        [
            'methods'             => 'POST',
            'permission_callback' => '__return_true',
            'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
                ...
                $url = rawlead_api_base_url() . '/v1/me/subscription/confirm';
                $response = wp_remote_post($url, [
                    'timeout' => 25,
                    'headers' => array_merge(
                        rawlead_api_user_headers($request, false),
    ...
    register_rest_route('rawlead/v1', '/me/subscription/cancel', [
        [
            'methods'             => 'POST',
            'permission_callback' => '__return_true',
            'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
                ...
                $url = rawlead_api_base_url() . '/v1/me/subscription/cancel';
                $response = wp_remote_post($url, [
                    'timeout' => 25,
                    'headers' => rawlead_api_user_headers($request, false),
                ]);
    ...
    register_rest_route('rawlead/v1', '/me/feed-prefs', [
        [
            'methods'             => 'GET',
            'permission_callback' => '__return_true',
            'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
                ...
                $url = rawlead_api_base_url() . '/v1/me/feed-prefs';
                $response = wp_remote_get($url, [
                    'timeout' => 20,
                    'headers' => rawlead_api_user_headers($request, false),
                ]);
    ...
    register_rest_route('rawlead/v1', '/support/ticket', [
        'methods'             => 'POST',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $body = $request->get_json_params();
            ...
            $url = rawlead_api_base_url() . '/v1/support/ticket';
            $response = wp_remote_post($url, [
                'timeout' => 20,
                'headers' => array_merge(
                    rawlead_api_support_headers($request),
                    ['Content-Type' => 'application/json']
                ),
                'body'    => wp_json_encode($body),
            ]);
    ...
    register_rest_route('rawlead/v1', '/support/thread', [
        'methods'             => 'GET',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $url = rawlead_api_base_url() . '/v1/support/thread';
            $response = wp_remote_get($url, [
                'timeout' => 20,
                'headers' => rawlead_api_support_headers($request),
            ]);
            if (is_wp_error($response)) {
                return $response;
            }
    ...
    register_rest_route('rawlead/v1', '/support/unread', [
        'methods'             => 'GET',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $url = rawlead_api_base_url() . '/v1/support/unread';
            $response = wp_remote_get($url, [
                'timeout' => 20,
                'headers' => rawlead_api_support_headers($request),
            ]);
    ...
    register_rest_route('rawlead/v1', '/ops/support/tickets', [
        'methods'             => 'GET',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $data = rawlead_api_get('/v1/admin/support/tickets', [], false, $request);
            if (is_wp_error($data)) {
                return $data;
            }
            return new WP_REST_Response($data, 200);
    ...
    register_rest_route('rawlead/v1', '/ops/support/tickets/(?P<id>\d+)/reply', [
        'methods'             => 'POST',
        'permission_callback' => '__return_true',
        'callback'            => static function (WP_REST_Request $request): WP_REST_Response|WP_Error {
            $id = (int) $request->get_param('id');
            ...
            $url = rawlead_api_base_url() . '/v1/admin/support/tickets/' . $id . '/reply';
            $response = wp_remote_post($url, [
                'timeout' => 20,
