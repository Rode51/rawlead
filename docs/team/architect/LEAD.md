...
**Очередь владельца (группы, приоритет):** [`OWNER_INTENT.md`](OWNER_INTENT.md) § «Бэклог владельца».

---

## Бэклог владельца — triage (обязательно)

Сообщение владельца в чат (**баг, идея, «убрать N»**) — **не команда «чинить сейчас»**, пока он сам не сказал «срочно / сейчас».

| Шаг | Lead |
|-----|------|
| 1 | Записать в [`OWNER_INTENT.md`](OWNER_INTENT.md) § **«Бэклог владельца»** — одна строка + **этап** (см. таблицу этапов ниже) |
| 2 | Выставить **приоритет** (P0–P2) и **волну** (A/B/C) — см. OWNER_INTENT |
| 3 | В **`CODER_PROMPT` / `DESIGNER_PROMPT`** — § только когда этап **активен** (в шапке «→ Сейчас @coder») |
| 4 | **`TASKS.md`** — одна строка-указатель, без копипаста ТЗ |
| 5 | **Спор:** если владелец просит P0, а по roadmap это P2 — аргумент + вопрос (см. `lead-architect.mdc` § Спор) |

**Этапы (куда класть задачи):**

| Этап | Когда трогаем | Примеры |
|------|----------------|---------|
| **E-now** | Текущая волна roadmap | SITE-POLISH, FEED-FRESHNESS, приёмка ленты |
| **E-vps** | После стабильной ленты на ПК / перед 24/7 без ПК | P5-E2-VPS |
| **E-polish** | ЛК, копирайт, мелкий UX | счётчик «7 дней», чипы, mobile |
| **E-3f** | Живая лента + ЛК на проде | ИИ-агент, L2 |
| **E-stress** | После SITE-POLISH | PRE-PROD-STRESS |
| **E-later** | После 1-го юзера / метрик | биллинг, freemium |

**Срочный баг** (сайт/лента/деньги лежат): тикет `docs/problems/` → `@mechanic` или § в `CODER_PROMPT` с пометкой **hotfix** — только по слову владельца «срочно».
...
| **Нарушение** | Владелец: `git checkout -- src/ scripts/ wordpress/` или откат коммита; Lead пишет `CODER_PROMPT.md` |

Lead **не** «помогает быстро» править Python/JS/PHP — даже пересылка TG, одна строка, hotfix, deploy-скрипт.

**Исключение (единственное):** владелец **в том же сообщении** явно пишет «Lead, залей hotfix сейчас» — иначе только `@coder` / `@mechanic`. После — строка в `STATUS`: «вне регламента» или «решение владельца».

**Прецеденты «Fix Lead: upload …» в старом STATUS** — исторические ошибки процесса, **не** разрешение кодить дальше.
...
| **Сообщение коммита** | **Только по-русски** — что сделали / приняли (1–2 предложения); Lead не пишет сухие английские `fix:` без пояснения |
| **Спор с владельцем** | Если идея не лучшая — аргумент + альтернатива + вопрос; после согласия — в канон. См. `lead-architect.mdc` § «Спор с владельцем» |
...
| Два источника (чат + файл) | В чат владельцу: **сводка простым языком** «что сделает Coder» + **копипаст** (см. `lead-architect.mdc` § Handoff) |
| **Пояснения архитектуры** | Формат: суть 2–3 предложения → функции с **вход / БД / фронт** → без воды (см. `lead-architect.mdc` § «Пояснения архитектуры») |
...
0. **`OWNER_INTENT.md`** — решения владельца + **§ Бэклог владельца** (группы, приоритет, этап); после каждой пачки задач из чата
...
| **Файлы** | Обязательны § **«Файлы»**, **«Не трогать»**, **«Лимиты»** — шаблон в `lead-architect.mdc` § **Архитектурный гард** · сверка с [`PROJECT_MAP.md`](../common/PROJECT_MAP.md) |
...
## Coder vs Mechanic (канон, 2026-05-27)

| | **Coder** | **Mechanic** |
|---|-----------|--------------|
| **Зачем** | План: фичи, блокеры PRE_PROD, рефактор по roadmap | Инцидент: сломалось / регресс / непонятный баг в рантайме |
| **Артефакт** | `team/architect/CODER_PROMPT.md` | `docs/problems/YYYY-MM-DD-*.md` |
| **Чат** | `@coder` · модель — как у владельца (часто Composer) | `@mechanic` · **Gemini 2.5 (~2M)** — можно разбирать весь repo |
| **Чтение** | Только промпт + `STATUS` | Тикет + логи/пути из тикета; при необходимости весь `src/`/`scripts/`/`desktop/` |
| **Git** | Не коммитит | Не коммитит → Lead |

### Алгоритм Lead

1. Есть готовый § в **`CODER_PROMPT`** и Coder **не закончил** → остаётся **Coder** (не открывать Mechanic на тот же §).
2. Владелец: «упало / дубли / не пишет в Neon / красное» + факты из лога → **тикет** → **Mechanic**.
3. Coder сдал, Lead verify **❌** дважды по одному симптому → **тикет** с фактами verify → Mechanic (эскалация).
4. Mechanic закрыл → в тикете **Решение**; Lead: при необходимости дописать `CODER_PROMPT` / `FOR_YOU` / закрыть дубль задачи в `TASKS`.
5. **Долго чиним**, корень не ясен, дальнейшие правки Coder **угрожают регрессом** → **тикет** → Mechanic (**дорого**, не на каждый чих).

**Владелец:** Mechanic — `@mechanic` + `@docs/problems/…` + модель **Gemini 2.5 (~2M, OpenRouter)**. Не подменять Mechanic-ом активный Coder-промпт без слова Lead. Coder/Lead сами предлагают эскалацию, когда «накололось» — владелец решает, открывать ли чат.
