...
Версия: **0.12** · Lead Product + владелец · 2026-06-03 (Premium 790 ₽ · O105 copy · O101 слоты)
...
| **Скорость (O11+O23)** | **anon** `/lenta/` — задержка **~15 мин**; **paid** — **мгновенно** на **`/lenta/`** (не отдельная лента в ЛК) |

### §0j — Две скорости + §0k Inbox ЛК (**O11 + O23**, владелец 2026-05-28)

**Не путать** с ops FEED-FRESHNESS (радар). **O11** — продуктовый gate на **`/lenta/`**.

| Кто | `/lenta/` | `/cabinet/` |
|-----|-----------|-------------|
| **Anon** | лента, **~15 мин** задержка, **без** «Написать отклик» | только вход / профиль (или редирект) |
| **Free (TG, не paid)** | **= anon:** **15 мин**, без «Написать отkлик» до Stars **✅ владелец 2026-05-28** | навыки, подписка в ЛК; **не** дубль ленты |
| **Paid (`is_active`)** | **мгновенно** + кнопка **«Написать отклик»** на карточке | **inbox:** заказы, по которым писали отклик; **удалить** из ЛК |

**ЛК ≠ вторая лента match.** Карточки match из O22/LK-FEED — **снять** с `/cabinet/` при O23.

**Coder:** § **CABINET-INBOX-O23** · § **FEED-DELAY-O11** · L2 prompt § **L2-REPLY-SCENARIO**.

**Copy/UI:** TWO-SPEEDS-COPY · LEAD_DESIGN — inbox ЛК.

### Direction C — WP `/cabinet` (**пересмотр O23**)

| | |
|--|--|
| **Было (до O23)** | match-лента = копия feed с фильтрами |
| **Стало** | **inbox откликов** + профиль (навыки, подписка, вход) |
| **Доступ** | TG-login; контент inbox — после «Написать отkлик» на `/lenta/` |
| **Действия** | просмотр черновика, copy, **удалить** из своего ЛК |

### Direction C (legacy note — архив до O23)
...
| **Что добавляется** | (1) на **`/lenta/`** (paid): **«Написать отклик»** → L2 → попадает в **inbox `/cabinet/`**; (2) push в TG при новом лиде; (3) пауза подписки |
| **Цена** | **790 ₽/мес** (СБП · USDT/TON) **или 300 ⭐ Stars** · зафиксировано 2026-06-03 · см. § O105 |
...
| **3** | **Подписка ИИ-агент** | Match по тегам + кнопка «Написать отклик» (черновик под профиль) + push в TG + пауза без штрафов | **790 ₽/мес** (+ Stars) · O101 лимит слотов · «Дешевле FL.ru PRO, умнее агрегаторов» | WP \/cabinet\ |
...
| **3h Биллинг (790 ₽/мес + Stars)** | → после E2E | СБП/crypto вручную (O105) · Stars live · ЮKassa позже |
...
| **3r Две скорости (O11)** | → после 3h биллинг | anon `/lenta/` delay ~15 мин · paid instant ЛК+TG |
...
- **Несколько тарифов** в MVP — один **RawLead Premium** **790 ₽/мес** (или 300 ⭐); тарифная сетка после первых платящих
...
_Lead Product + владелец 2026-06-03 (v0.12): **Premium 790 ₽** — решение владельца (OWNER_INTENT § O105). **Copy-pack O105-COPY — черновик Architect, PM не сдавал.**_
