...
Версия: **0.9** · Lead Product · 2026-05-24
...
| **2** | **Открытая лента** | Публичная лента на WP — все спарсенные заказы с ИИ-модерацией; фильтры, сортировки. Привлекает трафик, доказывает ценность | Любой посетитель сайта | **бесплатно, без регистрации** | 🟡 строим MVP |
...
```mermaid
flowchart LR
  subgraph sources["Открытые источники"]
    FL["FL.ru / Kwork"]
    TG["TG-чаты (acc1/2/3)"]
  end
  subgraph ingest["Ingest"]
...
| | |
|--|--|
| **Источники** | FL/Kwork + TG-чаты acc1/2/3 (всё, что радар достал) |
| **Фильтр** | без `is_visible` — владелец видит **всё** для проверки качества модерации |
...
|--|--|
| **Источники** | те же |
| **Фильтр** | `is_visible=true` (после ИИ-модерации) |
...
| **Что добавляется к /cabinet** | (1) автоматический расчёт **рыночной цены** заказа; (2) **черновик отклика** под профиль юзера; (3) **push в TG** при новом match'е (`telegram_chat_id` из профиля) |
| **Цена** | определяется после MVP — ориентир 300–990 ₽/мес |
...
| **2. Открытая лента** | **Любой фрилансер** (anon-посетитель сайта) | Цель — затащить, показать ценность бесплатно, конвертировать в регистрацию. ICP пока — «фрилансер на вайбкодинге», как владелец. |
| **3. Подписка ИИ-агент** | **Фрилансер, который реально закрывает заказы и хочет автоматизацию** | Готов платить за 30 минут экономии в день: цена, отклик, push при новом матче. Не «попробовать», а «упростить процесс». |
...
| **3** | **Подписка ИИ-агент** | Match по тегам + рыночная цена + черновик отклика + push в TG | **300–990 ₽/мес** (точная цена — после MVP) | WP `/cabinet` |
...
| **3b Neon SaaS-ready** | **→ активна** | schema (`raw_leads` + `users` + `user_tags` с `user_id`), миграция SQLite → Neon |
| **3c REST API + WP `/feed`** | **→ активна** | `/v1/feed`, `/v1/me`, `/v1/market` — открытая лента anon |
| **3d WP `/cabinet` single-user** | **→ активна** | `user_id=1` (владелец), match по тегам, `final_rank` в карточке |
| **3e Habr Career парсер** | **→ активна** | новый источник в радаре (только в MVP, без VK/Avito) |
| **3f ИИ-агент** | → после 3b–3e | рыночная цена + черновик отклика + push в TG для владельца |
| **3g TG Login Widget + регистрация** | → после ИИ-агента работает | multi-user; первая возможность для не-владельца зайти в `/cabinet` |
| **3h Биллинг (один тариф 300–500 ₽/мес)** | → после 1-го внешнего юзера | минимальный — ЮKassa или ручной перевод; не блокирует MVP |
...
**Под ставку B (Lead Product v0.9 2026-05-24):** активные фазы — **3b · 3c · 3d · 3e · 3f**. Финальные сроки, зависимости и порядок передаёт `@lead-architect` в `ROADMAP.md`. Lead Product **не правит ROADMAP** — это зона `@lead-architect`.

**Убрано:** mobile app · отдельный сайт-витрина · переговоры с площадками · юристы до выручки.
...
- **Несколько тарифов** в MVP — один «всё включено» 300–500 ₽/мес, тарифная сетка после первых платящих
...
_Владелец 2026-05-24 (v0.9, **смена ставки A → B**): **открытая площадка заказов** (anon-лента, бесплатно, без регистрации) + **подписка на ИИ-агент и анализ рынка** (300–500 ₽/мес, один тариф). Auth — TG Login Widget (отложен до multi-user). Источник модерации — **ИИ на ingest**, а не whitelist. Поле `contour` отменено. Парсим только открытые источники (без Habr API / VK переговоров / юристов сейчас). Архитектура SaaS-ready с дня 1 (user_id во всех таблицах). Витрина услуг и портфолио — **отложены до MVP**. Habr Career парсер — добавлен в MVP. Цель MVP — 5 пунктов в §0d. Triggers пересмотра — после MVP, либо 6 нед без платящих._
