...
| Блок | Суть |
|------|------|
| **O101 — лимит черновиков** | На один заказ — **K** персональных L3 (старт с 10). Потом кнопка отклика **серая**, карточка **остаётся в ленте**. На карточке: «осталось N из 10». Анти-тык: лимит в час на человека + слот только при нормальном match. **Не** аукцион и **не** «N фрилансеров смотрят» (отказ от Светофора O100). |
