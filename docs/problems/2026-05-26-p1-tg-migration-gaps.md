...
| P1.2 частично | `tg_monitor.filter_listen_chat_ids` — слушает только пересечение allowlist + **старый** TG-A JSON + `TG_JOIN_QUEUE.csv` (done) |
...
- [ ] Listen **только** allowlist (без старого TG-A) или JSON = PDF
- [ ] Join queue v2 → Tier A из allowlist
