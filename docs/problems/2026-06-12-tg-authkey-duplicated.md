...
| acc | session | статус |
|-----|---------|--------|
| acc1 | `+66953964608_telethon.session` | ✅ allowlist ok · `тг:монитор:acc1: ready` 15:00:52 UTC |
| acc2 | `+66967716330_telethon.session` | ✅ allowlist ok · `тг:монитор:acc2: ready` 15:00:53 UTC |
| **acc3** | **`233333925_telethon.session`** (+351924778041) | ❌ **`allowlist:acc3:connect:AuthKeyDuplicatedError`** с 14:32 UTC |

Лог: `grep allowlist …14:32` → acc1 ok · acc2 ok · **acc3 AuthKeyDuplicated**.

### t2 — Replace session ⚠️ blocked

1. Backup сгоревшего на VPS: `233333925_telethon.session.burned-2026-06-12` (40960 B)
2. Restore с ПК `Desktop\Parser\233333925_telethon.session` (36864 B, mtime до инцидента) → **тоже AuthKeyDuplicated** (ключ мёртв глобально у Telegram)
...
Owner path **2026-06-12:** `C:\Users\hramo\Desktop\Parser\233333925_telethon.session` — **AuthKeyDuplicated** on VPS (dead).

Owner **2026-06-12 ~15:14:** `C:\Users\hramo\Downloads\233333925.json` (547 B) — **JSON only**, no paired `.session` in Downloads · Telethon radar **needs `.session`**, not JSON alone (see `docs/ops/TELEGRAM_LOGIN.md` § «Что лежит у тебя»).
...
Нужна **свежая Session+Json** продавца для **233333925** (+351924778041):
...
4. scp новый `233333925_telethon.session` → `/opt/rawlead/data/sessions/` · `chown rawlead:rawlead`
5. Вернуть `TELETHON_MONITOR_ACCOUNTS=acc1,acc2,acc3` · restart radar · DoD: `тг:монитор:acc3: ready`

**Заметка:** на ПК сегодня появился `234694761_tdata.zip` (11:39) — это **acc4**, не acc3; сгорел именно acc3.
...
| **Старый acc3** | `233333925` — **снять**, ключ мёртв |
...
|-----|--------|
| stop radar | ✅ |
| backup dead `233333925*` | ✅ → `*.burned-2026-06-12-t4` |
...
2. Backup dead: `233333925_telethon.session*` → `*.burned-*` (if still present)
...
| backup dead session | `/opt/rawlead/data/sessions/233333925_telethon.session.burned-2026-06-12-t4` (+ nested `.burned-2026-06-12`) |
...
|-----|----------------------------|
| acc1 | `+66953964608_telethon.session` |
| acc2 | `+66967716330_telethon.session` |
| acc3 | `233333925_telethon.session` |
