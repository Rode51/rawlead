...
## Lead verify O194 (2026-06-13) — **ingest restored ✅**

| Check | Result |
|-------|--------|
| Deploy | ✅ `deploy-o194-youdo-detail-subprocess-vps.py` · radar **active** |
| Root cause | Listing subprocess (O190) OK · **in-process detail fetch** in `_process_listings` → asyncio crash after `health:youdo ok` |
| Fix | **Approach A:** `fetch_youdo_detail_snapshot` → `youdo_fetch_worker.py --stage detail` (camoufox subprocess) |
| Post-deploy fetch | ✅ **00:07:23** `fetch_end parsed=50` · `health:youdo ok` |
| Ingest | ✅ **`pipeline:L1 youdo:id=` ×29+** in first fetch cycle · **7×** `visible=1` |
| Asyncio | ✅ **0** on 2026-06-13 (was **12×** on 2026-06-12 pre-fix) |
| Cycle time | ⚠ YouDo-fetch cycle **~6–10 min** (50× detail subprocess + L1) — expected, not watchdog fail |
| Owner report | ✅ New tasks on YouDo today were missed pre-O194 · now entering pipeline |

**Paired stability:** FL **O193** subprocess listing unchanged · `health:fl ok` · L1 on new IDs (e.g. `5509273` @ 23:25 pre-O194 night).
