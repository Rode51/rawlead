...
**O104 — health:** после каждого цикла в `radar_site.log` / `radar.log` — строка `health:{источник}`: `status=ok downloaded=N new=M lag_p50=2m` или `status=fail kind=antibot` (kind: antibot, 403, browser, parse, proxy, timeout). Реестр в SQLite `exchange_health:*` · алерты 🔴 только @FLPARSINGBOT (cooldown 30 мин).
...
**TG-бот:** кнопка «ℹ Статус» и `/status` — один текст (§ BOT-STATUS-V2, O32). Site-пример:
...
Legacy — `@FLPARSINGBOT`, блок Neon consumer (без бирж). Блок «── Проблемы ──» только если есть сбои. Лимит **&lt; 3500** символов.
...
**L1 backlog без OpenRouter** (старый хвост):  
...
**Site — конвейер L1:** `конвейер:L1=N (batch≤40)` — пачка backlog drain при `L1_BACKLOG_DRAIN=1`.

**Site — YouDo:** `fetch:youdo proxy=…` · при антиботе — browser fallback (см. `youdo_parser.py`). Баны только `youdo:*` в SQLite `exchange_proxy_bans_v2`.

**TG acc2:** `тг:монитор:… join-bootstrap pending=N` · `join_auto=да` при `TG_JOIN_IN_TG_MAIN=1`.

Канон ops 2026-06-03: [`../problems/2026-06-03-ingest-l1-tg-youdo.md`](../problems/2026-06-03-ingest-l1-tg-youdo.md).
