#!/usr/bin/env python3
"""O262b: YouDo list-view trace + force click when map HTML has no cards."""
...
def _verify_remote() -> str:
    remote = r"""
grep 'youdo:trace stage=list_view' /opt/rawlead/data/radar_site.log | tail -5 || echo 'no list_view yet'
...
def main() -> int:
    for key, val in (
        ...
        if not _patch_env_key(key, val):
            ...
            return 1
    print("env: O262b list-view vars ok")
    ...
    if not ok:
        ...
        return 1
    print("OK — O262b deployed (grep stage=list_view after YouDo cycles)")
    return 0
