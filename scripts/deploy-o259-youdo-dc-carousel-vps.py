#!/usr/bin/env python3
"""O259: YouDo DC carousel — same 4 DC as FL, rotate on SPA before hard_reset / RU."""
...
from exchange_proxy import youdo_dc_alive_urls, youdo_dc_pool_size, youdo_ru_alive_urls
...
def main() -> int:
    ...
    dc_val = ",".join(dc)
    slots_n = str(len(dc))
    print(f"O259 DC pool: {len(dc)} slots — {', '.join(_hint(u) for u in dc)}")

    if not _patch_env_key("YOUDO_DC_PROXY_URLS", dc_val):
        print("FAIL — YOUDO_DC_PROXY_URLS not written")
        return 1
    if not _patch_env_key("YOUDO_O191_DC_SLOTS", slots_n):
        print("FAIL — YOUDO_O191_DC_SLOTS not written")
        return 1
    print("env: YOUDO_DC* ok")
    ...
    ok = "active" in (rst or "") and "youdo_dc_alive=" in verify
    if not ok:
        print("O259 DEPLOY CHECK FAILED")
        return 1
    print("O259 DEPLOY OK")
    return 0
