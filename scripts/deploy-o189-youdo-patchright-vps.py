#!/usr/bin/env python3
"""O189: YouDo listing via patchright (CDP Runtime.Enable patch) — PoC on VPS."""
...
def main() -> int:
    print("=== O189 deploy: patchright for YouDo only ===")
    remotes: list[str] = []
    for rel in _UPLOADS:
    ...
    deps = (
        "cd /opt/rawlead && sudo -u rawlead .venv/bin/pip install -q patchright && "
        "sudo -u rawlead .venv/bin/patchright install chromium"
    )
    _, out, _ = ssh.run(
        f"{deps} && "
        f"{env_cmds} && "
        "grep YOUDO_BROWSER=patchright /opt/rawlead/.env.site && "
        f"{reset_guard} && "
        ...
        "echo o189_patchright_deploy_ok",
        check=False,
    )
    text = out or ""
    print(text)
    if "o189_patchright_deploy_ok" not in text or "active" not in text:
        print("O189 DEPLOY CHECK FAILED")
        return 1
    print("O189 DEPLOY OK — run: python scripts/smoke_youdo_t6c_vps.py")
    return 0
