#!/usr/bin/env python3
"""O72e-L2-r4: deploy l3_human_style.py → restart rawlead-api."""
...
def main() -> int:
    print("=== O72e-L2-r4 deploy: l3_human_style.py ===")
    ...
    _, out, _ = ssh.run(
        ...
        "grep -c 'GOOD (#9843 GAS)' /opt/rawlead/src/l3_human_style.py && echo r4_ok",
        check=False,
    )
    print(out.strip())
    text = out or ""
    ok = "active" in text and "r4_ok" in text
    if ok:
        print("DEPLOY OK")
        return 0
