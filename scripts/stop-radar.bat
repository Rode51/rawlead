...
powershell -NoProfile -Command ^
  "Get-CimInstance Win32_Process | Where-Object { ($_.Name -eq 'python.exe' -or $_.Name -eq 'pythonw.exe') -and $_.CommandLine -match 'uisness[\\/](src[\\/](main|neon_legacy_consumer)|scripts[\\/](tg_main|tg_join_daemon|tg_join_queue))' } | ForEach-Object { Write-Host ('Kill PID ' + $_.ProcessId); Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"
