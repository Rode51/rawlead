...
# O72e-3: worst-3 judge fixtures — ≥2 группы терминов в draft (unit)
_O72E3_WORST_TERM_GROUPS: dict[int, tuple[tuple[str, ...], ...]] = {
    ...
    8776: (
        ...
        ("критическ", "critical", "отложен", "defer", "минификац"),
        ("pagespeed", "dom", "wp rocket", "lms", "ajax"),
    ),
    8925: (
        ("rhino", "рино"),
        ("google apps script", "apps script", "google таблиц", "таблиц"),
        ("es6", "миграц", "синтаксис", "api"),
    ),
}
...
def sanitize_l1_dev_design_marketing_tags(
    ...
    *,
    ...
    if integration_dev and "web_design" in tags and "wordpress_dev" not in tags:
        tags = [t for t in tags if t != "web_design"]
        if "api_integration" not in tags and len(tags) < _L1_MAX_TAGS:
            tags.append("api_integration")

    email_mkt = any(m in hay for m in ("e-mail", "email", "рассылк", "во входящие", "коммерческ"))
    blogger_mkt = any(m in hay for m in ("блогер", "размещен", "smm", "таргет", "авито"))
    if email_mkt or blogger_mkt:
        drop = {"api_integration", "python", "wordpress_dev"}
        tags = [t for t in tags if t not in drop]
        for add in ("smm", "target_ads", "chatbot_marketing"):
            if blogger_mkt and add == "smm" and add not in tags and len(tags) < _L1_MAX_TAGS:
                tags.append(add)
            if email_mkt and add == "chatbot_marketing" and add not in tags and len(tags) < _L1_MAX_TAGS:
                tags.append(add)
...
verdict — «Брать» | «Сомнительно» | «МИМО»
task_summary — 1–2 предложения: **что именно** нужно сделать (формат результата, платформа, ключевой стек из ТЗ); **всегда непустой**; минимум **2 конкретных факта** из описания (не «разработка по ТЗ», не копипаста заголовка)
lead_tags — max 6 canonical_tag из разрешённого списка v0.3 (51 тег); при МИМО — []; **только из ТЗ**, не додумывай стек
ai_reasons — массив 2–3 коротких строк «почему такой verdict»; каждая строка — **факт из ТЗ** (бюджет/платформа/тип работ); **всегда 2–3 строки**, не пустой массив
**Бюджет в task_summary/ai_reasons — только если явно в поле «Бюджет» или тексте заказа; не выдумывай суммы.**

Ниши (lead_tags отражают тип работы, не путай dev/design/marketing):
- **dev:** код, API, интеграции, WP-тема/плагин, оптимизация скорости (PageSpeed, Elementor, WooCommerce), парсинг, Rhino/Google Apps Script, настройка серверов, копия сайта с функционалом
- **design:** логотип, фирменный шрифт, UI/UX-макет, брендинг, иллюстрация — **без** программирования как основной задачи
- **marketing:** email-рассылки, SMM, блогеры, таргет, консультации по Авито/рекламе
- **text:** копирайт, художественные тексты, сценарии

Жёстко dev ≠ design:
- Аудит скорости WP+Elementor+WooCommerce+Tutor LMS → **wordpress_dev** (dev), не web_design
...
- Email-массовые КП «во входящие» → marketing; verdict **Сомнительно** (риски антиспама)
...
МИМО: список **вакансий** / подбор персонала без ТЗ на заказ — summary «вакансии, не фриланс-заказ», lead_tags [].
Сбор баз компаний/рекламодателей, лидов, email — ниша **marketing**, не dev.
Брать: парсеры, TG-боты, Python-автоматизация, правки бэкенда 1–2 файла; простой сайт/правки WP с ясным ТЗ.
...
reply_draft: как §1–§3 shared-agent — стек **из ТЗ**, 2–3 нишевых термина, шаги «Сначала…», один вопрос; **без срока/цены**. ЗАПРЕЩЕНО: «Готов…», «большой опыт», FastAPI/Neon без ТЗ; Cursor/ИИ/промпт. >6 предл. — warn."""
...
- В письме назови **≥2** инструмента/технологии **словами из ТЗ**.

§2 НИШЕВАЯ КОНКРЕТИКА (2–3 термина/проблемы из ТЗ, не шаблон «аудит»):
- WP/Elementor/WC/Tutor: критический CSS, отложенная загрузка JS, минификация; раздутый DOM Elementor; Ajax/видео Tutor LMS; WP Rocket / PageSpeed.
...
§3 СТРУКТУРА (4–5 предложений, **без срока, цены, «от X ₽»**):
(1) «Заинтересовал…» / «Беру задачу…» + **одна** деталь из ТЗ ·
(2–3) шаги «Сначала…, затем…» — **термин из ТЗ в каждом шаге** ·
(4) **один** уточняющий вопрос по пробелу в ТЗ (не «готов приступить», не «обсудить детали» без «?»).

ЗАПРЕЩЕНО: начало с «Готов…»; «Уважаемый заказчик», «Готов взяться», «Имею большой/огромный опыт», «качественно и в срок»;
...
- python, php, excel, google sheets, google apps script, rhino, figma, wordpress, elementor, mysql, 1c…
- Google Apps Script / Rhino V8 / Excel — если в тексте; **не** python, если заказ про GAS→Rhino
...
def _tz_stack_hints_for_reply(title: str, description: str, task_summary: str) -> str:
    ...
    if any(m in hay for m in ("rhino v8", "rhino", "google apps script", "google-таблиц")):
        hints.append(
            "⚠ Стек из ТЗ: Google Apps Script → Rhino V8 (JavaScript). "
            "Назови в письме Apps Script и Rhino; не подставляй Python."
        )
    if any(m in hay for m in ("elementor", "tutor lms", "woocommerce", "wp rocket", "pagespeed")):
        hints.append(
            "⚠ WP-стек: WordPress/Elementor/WooCommerce/Tutor — "
            "упомяни критический CSS, отложенную загрузку JS, DOM Elementor или Ajax LMS."
        )
