...
_CUSTOMER_II_TERM_RE = re.compile(
    ...
    rf"\bml[-/\s]|\bml\b|machine[\s-]learning|scikit[_-]learn|"
...
_REPLY_DRAFT_FALSE_TRUNC_RE = re.compile(
    r"обрыва(?:ется|ется)|"
    r"оборвал(?:ся|ось|ась)|"
    r"текст\s+обрыва|"
    r"фраза\s+«[^»]*…»|"
    r"«[^»]*…»\s*обрыва|"
    r"не\s+вижу\s+(?:конец|продолжен|остат)|"
    r"неполн(?:ое|ый)\s+т[з3]|"
    r"описание\s+обрез|"
    r"не\s+хватает\s+(?:контекст|части\s+т[з3])",
    re.IGNORECASE,
)
...
def _shared_snippet_for_l2(description: str) -> tuple[str, bool]:
    """O168 g1c: L2 «Описание заказа» block — may be shorter than full TZ."""
    desc = (description or "").strip()
    if len(desc) <= _MAX_SHARED_SNIPPET_CHARS:
        return desc, False
    return desc[: _MAX_SHARED_SNIPPET_CHARS - 1] + "…", True


def _reply_draft_false_truncation_reason(
    draft: str,
    description: str = "",
    *,
    snippet_truncated: bool | None = None,
) -> str | None:
    """Reject drafts that claim TZ is cut off when L2 prompt had the full snippet."""
    if not (draft or "").strip() or not (description or "").strip():
        return None
    if snippet_truncated is None:
        _, snippet_truncated = _shared_snippet_for_l2(description)
    if snippet_truncated:
        return None
    if _REPLY_DRAFT_FALSE_TRUNC_RE.search(draft):
        return "false_truncation_claim"
    return None
...
_TAKE_VERDICTS_CF = frozenset({"брать", "брат", "take", "ok"})
...
def validate_stored_l2_draft(
    *,
    ...
    trunc_reason = _reply_draft_false_truncation_reason(draft, description)
    if trunc_reason:
        fails.append(f"L2:{trunc_reason}")
    return fails
...
def _finalize_lite_analysis(
    ...
    *,
    ...
    if result.feed_visible and is_physical_service_job(title, snippet):
        forced = physical_service_lite_analysis(title=title, body=snippet)
        if forced is not None:
            return forced
...
- «Скрипт холодной рассылки / лидgen в Telegram-чатах» → marketing · chatbot_marketing, crm_marketing (не dev)
- «Слайды и инфографика для карточек товаров на WB/Ozon» → design · infographic_design, presentation_design (не marketing)
- «Карточки мебели на маркетплейсе: visual, A+, фото» → design · infographic_design, illustration (не ui_ux, не landing_page_design)
- «Схема процесса / mind-map в XMind по ТЗ» → text · technical_writing, ux_writing (не dev)
- «Гидроборт Москва–Городец, междугородные перевозки» → feed_visible **false** · summary «физическая услуга, не digital-фриланс» · lead_tags []
"""
...
lead_tags — **2–5 canonical_tag** на заказ (не меньше 2 при feed_visible=true), max 6 **только из словаря primary_category**; при feed_visible=false — []; **только из ТЗ**, не додумывай стек
...
5. холодная рассылка / лидgen в Telegram → **marketing** (chatbot_marketing), не dev/python
6. инфографика / слайды / visual для карточек товаров → **design** (infographic_design), не marketing
7. карточки физтоваров (мебель, Ozon/WB) → infographic_design/illustration, **не** ui_ux/landing_page_design
...
**PHYSICAL (O223) — feed_visible=false ВСЕГДА:** гидроборт, перевозка, грузчики, междугород, эвакуатор, погрузка, паллеты, рохля, курьерская доставка (не API/CRM) — summary **ровно** «физическая услуга, не digital-фриланс», lead_tags [].
...
# O170: дополнение к L1 system-prompt для TG-источников (few-shot реклама услуг)
_LITE_TG_APPEND = """
--- TG-источник (ужесточённый режим) ---
Этот пост из Telegram-чата. Применяй строже:

feed_visible=true ТОЛЬКО если есть ЯВНЫЙ признак «заказчик ищет исполнителя»:
  маркеры спроса (нужен/нужна/нужно/требуется/ищем + задача/платформа)
  И хотя бы один из: бюджет / срок / ТЗ.

feed_visible=false если:
  — пост рекламирует услуги («мы делаем», «наша команда», «наша студия», «под ключ»,
    «принимаем заявки», «пишите в лс», «стоимость от», «примеры работ», «кейсы»)
  — инфобиз/воронка: курс, марафон, наставничество, лид-магнит, прогрев
  — нет бюджета И нет срока И нет ТЗ — только описание чужих работ или услуги
  — мотивационный/продающий текст без конкретного ТЗ заказчика
  — scarcity («осталось N мест», «записывайтесь»)

Few-shot TG:
- «Книга под ключ — написание, вёрстка, автографы» → feed_visible=false (продаёт услугу)
- «Нужен разработчик Telegram-бота. Бюджет 15к, срок 2 нед.» → feed_visible=true
- «SMM-воронка для твоего бизнеса — пиши в лс» → feed_visible=false (реклама)
- «Ищем дизайнера для Tilda. ТЗ есть, бюджет обсуждается» → feed_visible=true
- «Ищешь каналы для рекламы? Помогу с продвижением» → feed_visible=false (продаёт услугу)
- «Наша команда делает сайты под ключ. Примеры работ в профиле» → feed_visible=false
- «Требуется верстальщик, оплата 5000 ₽, дедлайн пятница» → feed_visible=true
"""
...
_LITE_RETRY_MIN_TAGS_HINT = (
    "Повтор: верни минимум 2 canonical_tag из словаря primary_category (need ≥2 canonical_tag)."
)
...
def analyze_lite(
    ...
    *,
    ...
    from vacancy_filter import physical_service_lite_analysis, vacancy_lite_analysis
    ...
    pre = physical_service_lite_analysis(title=title, body=snippet or "")
    if pre is not None:
        return _finalize_lite_analysis(pre, title=title, snippet=(snippet or title or "").strip())
    ...
    user_base = _build_lite_user_message(
        title=title,
    ...
    # O170: TG-источник — ужесточённый system-prompt
    l1_system = _LITE_SYSTEM + _LITE_TG_APPEND if is_tg else _LITE_SYSTEM
    ...
    for attempt in range(2):
        try:
            ...
            tag_mismatch = bool(result.lead_tags and not finalized.lead_tags)
            tag_thin = len(finalized.lead_tags) < 2
            need_retry = (
                attempt == 0
                and finalized.feed_visible
                and (tag_mismatch or tag_thin)
            )
            if need_retry:
                logger.info(
                    "L1 %s tags — retry 1×",
                    "mismatch" if tag_mismatch else "thin",
                )
                user = f"{user_base}\n\n{_LITE_RETRY_MIN_TAGS_HINT}"
                continue
            # O170 post-L1 guard: TG + feed_visible=True, но нет маркеров заказа
            if is_tg and finalized.feed_visible:
                from tg_spam_filter import is_tg_order_post

                if not is_tg_order_post(title, snip):
                    logger.info(
                        "O170 tg_order_guard: feed_visible forced False — no order markers"
                    )
                    finalized = AiLiteAnalysis(
                        feed_visible=False,
                        task_summary=finalized.task_summary or "TG-реклама услуг — нет маркеров заказа",
                        lead_tags=(),
                        pending_tags=(),
                        ai_reasons=(
                            "TG post-L1 guard: нет маркеров заказа (нужен/требуется/бюджет/ТЗ)",
                            "Вероятно реклама услуг или self-promo",
                        ),
                        complexity=1,
                        primary_category=finalized.primary_category,
                    )
            return finalized
        except (
            AiAnalyzeError,
