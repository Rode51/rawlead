...
def fetch_html_playwright(
    ...
    *,
    ...
    proxy = _playwright_proxy(proxy_url or os.getenv("FREELANCEHUNT_PROXY_URL", "").strip())
    ...
    if "cloudflare" in low and ("blocked" in low or "challenge" in low):
        raise HtmlFetchError(
            "Cloudflare блокирует IP — задайте FREELANCEHUNT_API_TOKEN "
            "(https://freelancehunt.com/my/api) или FREELANCEHUNT_PROXY_URL."
        )
